...
const CcoContribution = React.memo(({
  ...
  useEffect(() => {
    const setup = async (ccoType) => {
      ...
      setRoundData({
        ...
        now,
        ccoToken,
        endTime: `${Number(configData.raiseStartTime) + Number(configData.duration)}`,
        beforeRaise: Number(daoMetaData.boosts[ccoType].metadata.raiseStartTime) > now,
        raiseOpen: Number(configData.raiseStartTime) < now && Number(`${+configData.raiseStartTime + +configData.duration}`) > now,
        raiseOver: `${Number(configData.startTime) + Number(configData.duration)}` < now,
        claimPeriodStartTime:
          daoMetaData.boosts[ccoType].metadata.claimPeriodStartTime,
        claimOpen: Number(daoMetaData.boosts[ccoType].metadata.claimPeriodStartTime) < now,
      });
    };
  ...
  useEffect(() => {
    if (roundData && address && daoProposals && daoProposals.length) {
      ...
      setCurrentContributionData({
        ...
        addressTotal,
        statusPercentage:
          (contributionTotal / +roundData.maxTarget) * 100,
        remaining: +roundData.maxTarget - contributionTotal,
        addressRemaining:
          +roundData.maxContribution - addressTotal,
      });
  ...
  if (roundData && roundData.active) {
    return (
      <MainViewLayout header={daoMetaData?.name} isDao isDaosquare={daoMetaData.daosquarecco}>
        <Box w='100%' position='relative'>
          <Flex wrap='wrap'>
